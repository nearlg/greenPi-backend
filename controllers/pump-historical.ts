...
function validateDependencies(
  ...
  return pumpRepository.find(pumpId).then(() => pumpHistorical);
}

function byEnvironmentId(
  req: Request,
  res: Response,
  next: Next
): Promise<PumpHistorical[]> {
  ...
  return checkQuery(commQuery, req.query).then(() => {
    ...
    return environmentRepository
      .find(environmentId)
      .then(environment =>
        pumpHistoricalRepository.findAllByPumps(
...
function byPumpId(
  ...
  return checkQuery(commQuery, req.query).then(() => {
    ...
    return pumpRepository
      .find(pumpId)
      .then(() =>
        pumpHistoricalRepository.findAllByPumpId(pumpId, sortBy, gte, lte)
      );
  });
}
...
function byPump(
  ...
  return checkQuery(commQuery, req.query).then(() => {
    ...
    let lte = req.query.lte ? new Date(req.query.lte) : null;
    let sortBy: string = req.query.sortBy;
    return pumpValidator
      .validate(req.query.pump, true)
      .then(pump =>
        pumpRepository
          .find(pump.id)
          .then(() =>
            pumpHistoricalRepository.findAllByPump(pump, sortBy, gte, lte)
          )
      );
  });
}
