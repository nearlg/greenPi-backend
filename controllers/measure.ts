...
function validateDependencies(measure: Measure): Promise<Measure> {
  ...
  return sensorRepository.find(sensorId).then(() => measure);
}
...
function byEnvironmentId(req: Request, res: Response, next: Next) {
  ...
  return checkQuery(commQuery, req.query).then(() => {
    ...
    return environmentRepository
      .find(environmentId)
      .then(environment =>
        measureRepository.findAllBySensors(
...
function bySensorId(req: Request, res: Response, next: Next) {
  ...
  return checkQuery(commQuery, req.query).then(() => {
    ...
    return sensorRepository
      .find(sensorId)
      .then(() =>
        measureRepository.findAllBySensorId(sensorId, sortBy, gte, lte)
      );
  });
}
...
function bySensor(req: Request, res: Response, next: Next) {
  ...
  return checkQuery(commQuery, req.query).then(() => {
    ...
    return sensorValidator
      .validate(req.query.sensor, true)
      .then(sensor =>
        sensorRepository
          .find(sensor.id)
          .then(() =>
            measureRepository.findAllBySensor(sensor, sortBy, gte, lte)
          )
      );
  });
}
