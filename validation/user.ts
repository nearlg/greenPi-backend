...
import * as userRegex from "@/validation/rules/user";
import {
  regexValidation,
  createError,
  rejectIfNull
...
export function validateFacebook(facebookId?: string): Promise<null | string> {
  if (!facebookId) {
...
export function validateGoogle(googleId?: string): Promise<null | string> {
  if (!googleId) {
    return Promise.resolve(null);
  }
  return regexValidation(
...
export function validateRoleName(roleName: RoleName): Promise<RoleName> {
  if (roleName) {
...
export function validate(user: User, checkId: boolean = true): Promise<User> {
